...
export default function FragilePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Client-side filter: frag_score > 0.7 OR state === "BROKEN"
      const filtered = resp.rows.filter(
        (r) => (r.frag_score !== null && r.frag_score > 0.7) || r.state === "BROKEN"
      );
      setRows(filtered);
  ...
  if (loading) {
    return <div style={{ padding: "40px", textAlign: "center", color: "var(--text-tertiary)" }}>Loading fragile data…</div>;
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "26px", fontWeight: 400, margin: 0 }}>Fragile</h1>
          <div style={{ fontSize: "13px", color: "var(--text-secondary)", marginTop: "4px" }}>
            Elevated downside risk — high fragility or broken state
          </div>
        </div>
                  ...
                  >
                    ...
                    <td style={{ padding: "10px 12px" }}>
                      <span style={{
                        ...
                        background: stock.state === "LEADER" ? "var(--rag-green-100)" : stock.state === "EMERGING" ? "var(--rag-amber-100)" : stock.state === "WEAKENING" ? "var(--rag-red-100)" : stock.state === "BROKEN" ? "var(--rag-red-100)" : "var(--bg-inset)",
                        color: stock.state === "LEADER" ? "var(--rag-green-700)" : stock.state === "EMERGING" ? "var(--rag-amber-700)" : stock.state === "WEAKENING" ? "var(--rag-red-700)" : stock.state === "BROKEN" ? "var(--rag-red-800)" : "var(--text-secondary)",
                      }}>
                        {stock.state ?? "—"}
                      </span>
                    </td>
      ...
      {!error && filteredRows.length === 0 && (
        <div style={{ padding: "40px", textAlign: "center", color: "var(--text-tertiary)" }}>No fragile signals found.</div>
      )}
    </div>
  );
}
