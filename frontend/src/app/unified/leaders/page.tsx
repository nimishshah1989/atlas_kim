...
export default function LeadersPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [strong, acc] = await Promise.all([
          postScreen("STRONG_ACCUMULATE"),
          postScreen("ACCUMULATE"),
        ]);
        if (cancelled) return;
        const all = [...strong.rows, ...acc.rows];
        setRows(all);
        setDataAsOf(strong.meta?.data_as_of ?? acc.meta?.data_as_of ?? null);
      } catch {
        setRows([]);
      } finally {
        setLoading(false);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
        <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "26px", fontWeight: 400, margin: 0 }}>Leaders</h1>
        <DataFreshness dataAsOf={dataAsOf} />
      </div>

      <div style={{ fontSize: "13px", color: "var(--text-secondary)" }}>
        STRONG_ACCUMULATE + ACCUMULATE candidates sorted by RS rank.
      </div>
          ...
          {rows.map((stock) => (
            <EvidenceCard
              ...
              metrics={{
                ...
                above_ema_20: null, above_ema_50: stock.above_ema_50,
                golden_cross: null,
                rsi_14: stock.rsi_14,
                macd: null, macd_signal: null,
                vol_21d: null, max_dd_252d: null,
                rs_nifty_1d_rank: null, rs_nifty_1w_rank: null, rs_nifty_1m_rank: null,
                rs_nifty_3m_rank: stock.rs_nifty_3m_rank,
                rs_nifty_6m_rank: null, rs_nifty_12m_rank: stock.rs_nifty_12m_rank,
                ...
                state: stock.state,
                action: stock.action,
                action_confidence: stock.action_confidence,
                frag_score: stock.frag_score,
                frag_level: null,
              }}
              narrative={null}
              dataAsOf={dataAsOf}
