...
export default function EmergingPage() {
  ...
  const [minRsFilter, setMinRsFilter] = useState<number>(0);
  const [capFilter, setCapFilter] = useState<string>("");
  const [includeSmallCap, setIncludeSmallCap] = useState(false);

  const [sortField, setSortField] = useState<SortField>("rs_nifty_3m_rank");
  ...
  const load = useCallback(async () => {
    ...
    try {
      const resp = await postApi<ScreenerResponse>("/api/unified/screen", {
        filters: [
          { field: "state", op: "eq", value: "EMERGING" },
  ...
  const filteredRows = useMemo(() => {
    ...
    if (minRsFilter > 0) {
      data = data.filter((r) => (r.rs_nifty_3m_rank ?? 0) >= minRsFilter);
    }
    if (capFilter) {
    ...
    return data;
  }, [rows, sectorFilter, minRsFilter, capFilter, includeSmallCap, sortField, sortDirection]);
  ...
  if (loading) {
    return <div style={{ padding: "40px", textAlign: "center", color: "var(--text-tertiary)" }}>Loading emerging stocks…</div>;
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "26px", fontWeight: 400, margin: 0 }}>Emerging</h1>
          <div style={{ fontSize: "13px", color: "var(--text-secondary)", marginTop: "4px" }}>
            Stocks improving RS trajectory — watch for entry
          </div>
        </div>
      ...
      <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <span style={{ fontSize: "12px", color: "var(--text-secondary)" }}>Min RS 3M:</span>
          <input
            type="range"
            min={0}
            max={100}
            value={minRsFilter}
            onChange={(e) => { setMinRsFilter(Number(e.target.value)); setPage(0); }}
            style={{ width: "120px" }}
          />
          <span style={{ fontSize: "12px", color: "var(--text-primary)", fontVariantNumeric: "tabular-nums", minWidth: "28px" }}>{minRsFilter}</span>
        </div>

        <select
          value={capFilter}
          onChange={(e) => { setCapFilter(e.target.value); setPage(0); }}
          style={{
            padding: "6px 10px", borderRadius: "6px", border: "1px solid var(--border-default)",
            background: "var(--bg-surface)", fontSize: "13px", color: "var(--text-primary)", minWidth: "120px",
          }}
        >
      ...
      {!error && filteredRows.length === 0 && (
        <div style={{ padding: "40px", textAlign: "center", color: "var(--text-tertiary)" }}>No emerging stocks found.</div>
      )}
    </div>
  );
}
