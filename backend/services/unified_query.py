...
async def get_cohort_aggregate(
    ...
    cache_key = _cache_key("aggregate", tenant_id, cohort_type, benchmark, period, str(snapshot_date))
    ...
    # Use raw SQL for median / percentile calculations — PostgreSQL percentile_cont
    sql = text(f"""
    SELECT
